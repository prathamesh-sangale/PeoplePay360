...
def list_attendance(
    ...
    for a in records:
        ...
        results.append({
            ...
            "check_in_time": a.check_in.strftime("%H:%M:%S") if a.check_in else "--:--",
            "check_out_time": a.check_out.strftime("%H:%M:%S") if a.check_out else "--:--",
...
def get_attendance_summary(db: Session = Depends(get_db)):
    ...
    avg_hours = db.query(func.avg(Attendance.worked_hours)).scalar() or 0.0
